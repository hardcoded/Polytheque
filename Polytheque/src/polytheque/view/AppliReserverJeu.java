...
public class AppliReserverJeu extends JPanel implements ActionListener {
	...
	private void creerPanneauDate() {
		...
		JLabel labelDate = new JLabel("Cliquez sur la date � laquelle vous voudriez emprunter le jeux :");
		...
		if (boutonSelectionne == this.boutonValider && this.dateChooser.getDate() != null && this.searchContent.getText() != null && this.dateChooser.getDate().after(currentDate)) {
			...
			if (NbExemplaires > NbReserves) {
				...
				if(this.tacheDAffichageDeLApplication.createReservation2(reservation,IdAdherent,IdJeu,dateReservationJeu)) {
					this.tacheDAffichageDeLApplication.afficherMessage("R�servation de jeu confirm�e !"," Confirmation", JOptionPane.INFORMATION_MESSAGE);
					this.tacheDAffichageDeLApplication.afficherAccueil();
				}
			}
			else {
				this.tacheDAffichageDeLApplication.afficherMessage("Cette extention n'est plus disponible, veuillez en choisir un autre !"," Probl�me ", JOptionPane.INFORMATION_MESSAGE);
			}
		}
		else {
